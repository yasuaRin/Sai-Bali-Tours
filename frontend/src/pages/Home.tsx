...
import { Loader2, Sparkles, ArrowRight, Star, ShieldCheck, Calendar, ArrowUpRight, TrendingUp, Tag } from 'lucide-react';
import { 
  getSignatureToursByCategory,
  getPopularTours,
  getPackages 
...
const Home: React.FC = () => {
  ...
  // Get featured blog post from Supabase
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Process signature tours to ensure category is a string
        const processedSignature = signatureData.map(tour => ({
          ...tour,
          category: typeof tour.category === 'object' ? (tour.category as any).name || 'Tour' : String(tour.category)
        }));
        
        const processedPopular = popularData.map(tour => ({
          ...tour,
          category: typeof tour.category === 'object' ? (tour.category as any).name || 'Tour' : String(tour.category)
        }));
        
        setSignatureTours(processedSignature);
        setPopularTours(processedPopular);
...
{signatureTours.length > 0 && (
  <section id="signature-tours" className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32 bg-gradient-to-b from-white to-gray-50/30 relative overflow-hidden">
    ...
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[4rem] xs:text-[6rem] sm:text-[8rem] md:text-[10rem] lg:text-[12rem] xl:text-[14rem] font-serif text-brand-orange/[0.03] select-none pointer-events-none whitespace-nowrap rotate-12" aria-hidden="true">
      ᬲᬿ ᬩᬮᬶ
    </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div className="text-center mb-12 sm:mb-16 md:mb-20">
        ...
        <div className="mt-6 sm:mt-8 md:mt-10">
          ...
          >
            ...
            <ArrowRight size={14} sm:size={15} md:size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </div>

      {/* Signature Tours Grid */}
      ...
      {/* Blog & Insights Section - Using Supabase data */}
      {featuredHomePost && (
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="max-w-3xl mx-auto text-center mb-12 md:mb-16 lg:mb-20 space-y-6 sm:space-y-8">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-400 font-medium leading-relaxed max-w-xl sm:max-w-2xl mx-auto">
                Beyond tourist spots, our editors uncover Bali's true culture, safety, and hidden sides.              </p>
            </div>
            ...
            <div className="max-w-5xl mx-auto relative">
              ...
              {/* Featured Post - Large Interactive Card */}
              <div className="group cursor-pointer animate-fadeIn relative">
                ...
                <Link to={`/blog/${featuredHomePost.slug}`} className="block relative h-[280px] sm:h-[350px] md:h-[450px] lg:h-[500px] rounded-[2rem] sm:rounded-[3rem] md:rounded-[4rem] overflow-hidden shadow-2xl border border-white/10 bg-brand-anchor">
                  <img 
                    src={featuredHomePost.image || 'https://gmcyxgjmlrytrwqgrona.supabase.co/storage/v1/object/public/website-assets/blog/blog-4-cover.png'} 
                    ...
                    <div className="max-w-2xl space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] md:tracking-[0.35em] lg:tracking-[0.4em]">
                        ...
                        <span className="text-white/60 flex items-center space-x-1 sm:space-x-2">
                          <Calendar size={8} sm:size={10} md:size={12} />
                          <span>{new Date(featuredHomePost.published_at || featuredHomePost.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
                        </span>
                      </div>
                      ...
                      <div className="pt-2 sm:pt-3 md:pt-4 lg:pt-6 xl:pt-8 flex items-center space-x-3 sm:space-x-4 md:space-x-6 lg:space-x-8">
                        <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3 lg:space-x-4 text-white font-black text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] xl:text-xs uppercase tracking-widest group/btn">
                          <span className="border-b-2 border-brand-orange/50 pb-0.5 sm:pb-1 md:pb-1.5 lg:pb-2 group-hover/btn:border-brand-orange transition-colors">Read Full Story</span>
                          <div className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 xl:w-14 xl:h-14 rounded-full border border-white/20 flex items-center justify-center group-hover/btn:bg-brand-orange group-hover/btn:border-brand-orange transition-all duration-500 shadow-xl">
                            <ArrowUpRight size={10} sm:size={12} md:size={14} lg:size={16} xl:size={18} className="text-brand-orange group-hover/btn:text-white group-hover/btn:rotate-45 transition-transform" />
                          </div>
                        </div>
              ...
              <div className="mt-8 sm:mt-10 md:mt-12 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6 md:gap-8 border-t border-white/5 pt-6 sm:pt-8 md:pt-10 lg:pt-12">
                ...
                >
                  <span className="text-white text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] xl:text-xs font-black uppercase tracking-widest">Explore Journal Archive</span>
                  <ArrowRight size={10} sm:size={12} md:size={14} lg:size={16} className="text-brand-orange group-hover/all:text-white group-hover/all:translate-x-1 transition-all" />
                </Link>
              </div>
            </div>
          </div>
                  ...
                  <div className="absolute bottom-3 sm:bottom-4 md:bottom-5 lg:bottom-6 left-3 sm:left-4 md:left-5 lg:left-6">
                    <div className="bg-white/10 backdrop-blur-md border border-white/20 p-2 sm:p-3 md:p-4 lg:p-5 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl text-white">
                      <div className="flex items-center space-x-1 sm:space-x-2 mb-0.5 sm:mb-1 text-brand-accent">
                        <ShieldCheck size={10} sm:size={12} md:size={14} lg:size={16} />
                      ...
                      <p className="text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] xl:text-xs font-medium text-white/80">Private vehicle, driver, and hotel inclusions standard.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 sm:space-y-5 md:space-y-6 lg:space-y-8 xl:space-y-10 order-1 lg:order-2">
                <div className="space-y-2 sm:space-y-3 md:space-y-4 lg:space-y-5">
                  <div className="flex items-center space-x-2 text-brand-orange">
                    <Star size={12} sm:size={13} md:size={14} fill="currentColor" />
                    <span className="text-brand-orange font-black uppercase tracking-[0.3em] sm:tracking-[0.35em] md:tracking-[0.4em] text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px]">Featured Service</span>
                  </div>
                ...
                <div className="bg-white p-4 sm:p-5 md:p-6 lg:p-8 xl:p-10 rounded-xl sm:rounded-2xl md:rounded-[2rem] lg:rounded-[2.5rem] xl:rounded-[3rem] border border-gray-100 shadow-md sm:shadow-lg md:shadow-xl lg:shadow-2xl space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6">
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-2 sm:gap-3">
                    <div>
                      <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-black text-brand-text mb-0.5 sm:mb-1 tracking-tight">{featuredPackage.title}</h3>
                      <div className="flex items-center space-x-2 text-brand-orange text-[8px] sm:text-[9px] md:text-[10px] lg:text-xs font-bold uppercase tracking-widest">
                        <Calendar size={10} sm:size={11} md:size={12} />
                        <span>{featuredPackage.duration_nights} Nights / {featuredPackage.duration_days} Days</span>
                      </div>
                    </div>
                    <div className="text-right w-full sm:w-auto">
                      <span className="block text-[6px] sm:text-[7px] md:text-[8px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Price From</span>
                      <span className="text-base sm:text-lg md:text-xl lg:text-2xl font-black text-brand-orange">${featuredPackage.price_estimate}</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-2 sm:pt-3 flex flex-col sm:flex-row gap-2 sm:gap-3">
                    ...
                    >
                      ...
                      <ArrowRight size={12} sm:size={13} md:size={14} className="group-hover:translate-x-1 transition-transform" />
                    </Link>
                    // src/pages/Home.tsx (line 350-360)
                      <a 
                      ...
                      >
                        <ArrowUpRight size={12} sm:size={14} md:size={16} lg:size={18} />
                      </a>
                  </div>
                </div>
              </div>
