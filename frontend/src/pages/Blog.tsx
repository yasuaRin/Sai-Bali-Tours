...
import { Loader2, Sparkles, ArrowRight, Calendar, ChevronRight, Search, Tag, Filter, Clock, User } from 'lucide-react';
...
const Blog: React.FC = () => {
  ...
  const getPrimaryImage = (post: BlogPost) => {
    if (post.images && post.images.length > 0) {
      const primary = post.images.find(img => img.is_primary);
      return primary?.image_url || post.images[0].image_url;
    }
    // Fallback based on post ID
    const fallbacks: Record<number, string> = {
      1: `${BUCKET_URL}/blog-1.png`,
      2: `${BUCKET_URL}/blog-2.jpg`,
      3: `${BUCKET_URL}/blog-3.jpg`,
      4: `${BUCKET_URL}/blog-4.png`
    };
    return fallbacks[post.id] || `${BUCKET_URL}/fallback.jpg`;
  };
          ...
          <div className="max-w-3xl space-y-6 sm:space-y-8">
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 text-brand-orange">
                <Tag size={12} sm:size={14} className="fill-current" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5 md:py-6">
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 md:gap-8 items-center justify-between">
            ...
            <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar w-full lg:w-auto pb-1 lg:pb-0">
              <div className="flex items-center space-x-1 sm:space-x-2 pr-3 sm:pr-4 border-r border-gray-100 mr-2 sm:mr-3">
                <Filter size={14} sm:size={16} className="text-gray-400" />
            ...
            <div className="relative w-full lg:w-80 xl:w-96 group">
              <Search className="absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-brand-orange transition-colors" size={16} sm:size={18} />
              <input
                type="text"
            ...
            <div className="mb-16 sm:mb-20 md:mb-24 lg:mb-32">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 md:gap-12 items-center">
                <div className="lg:col-span-7 relative group cursor-pointer">
                  <div className="relative aspect-[16/10] md:aspect-[16/9] lg:aspect-square rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-xl sm:shadow-2xl">
                    <img 
                    ...
                    />
                    <div className="absolute inset-0 bg-brand-anchor/20 group-hover:bg-transparent transition-colors duration-700"></div>
                  </div>
                  <div className="absolute -bottom-4 sm:-bottom-5 md:-bottom-6 -right-4 sm:-right-5 md:-right-6 w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 bg-brand-orange rounded-full flex items-center justify-center text-white shadow-2xl rotate-12 group-hover:rotate-0 transition-transform duration-500">
                ...
                <div className="lg:col-span-5 space-y-5 sm:space-y-6 md:space-y-8">
                  ...
                  <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-black text-brand-text tracking-tighter leading-[0.95] hover:text-brand-orange transition-colors cursor-pointer">
                    {featuredPost.title}
                  </h2>
                  ...
                  <div className="pt-2 sm:pt-3 md:pt-4">
                    <Link to={`/blog/${featuredPost.slug}`} className="group flex items-center space-x-4 sm:space-x-5 md:space-x-6">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-brand-anchor flex items-center justify-center text-white group-hover:bg-brand-orange transition-colors duration-300 shadow-xl">
                        <ArrowRight size={18} sm:size={20} md:size={22} lg:size={24} />
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16">
            {filteredPosts.length > 0 ? (
              filteredPosts
                ...
                .map((post, idx) => (
                <article key={post.id} className="group flex flex-col h-full animate-fadeInUp" style={{ animationDelay: `${idx * 100}ms` }}>
                  <div className="relative aspect-[4/3] rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[2.5rem] overflow-hidden mb-6 sm:mb-8 md:mb-10 shadow-md sm:shadow-lg cursor-pointer">
                    <img 
                  ...
                  <div className="flex-grow space-y-4 sm:space-y-5 md:space-y-6">
                    ...
                    <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-black text-brand-text tracking-tight group-hover:text-brand-orange transition-colors line-clamp-2 cursor-pointer leading-tight">
                      {post.title}
                    </h3>
                  ...
                  <div className="mt-6 sm:mt-8 md:mt-10 pt-4 sm:pt-5 md:pt-6 border-t border-gray-100 flex items-center justify-between">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                      <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden">
                        <User size={12} sm:size={14} md:size={16} className="text-brand-orange" />
                      </div>
                      <div className="flex flex-col">
                    ...
                    <Link to={`/blog/${post.slug}`} className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full border border-gray-100 flex items-center justify-center text-brand-anchor hover:bg-brand-orange hover:border-brand-orange hover:text-white transition-all">
                      <ChevronRight size={14} sm:size={16} md:size={18} />
                    </Link>
                  </div>
                </article>
              ))
              ...
              <div className="col-span-full py-20 sm:py-24 md:py-32 text-center">
                <div className="max-w-md mx-auto space-y-6 sm:space-y-8">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto border border-gray-100">
                    <Search size={20} sm:size={24} md:size={28} lg:size={32} className="text-gray-300" />
                  </div>
          ...
          {visibleCount < filteredPosts.length && (
            <div className="flex justify-center mt-12 sm:mt-16 md:mt-20">
              ...
              >
                ...
                <ChevronRight size={10} sm:size={12} md:size={14} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
          )}
